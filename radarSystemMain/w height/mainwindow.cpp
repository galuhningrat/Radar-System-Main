...
void MainWindow::processRadarData(const QString &data) {
    ...
    if (parts.size() == 3) { // Pastikan ada 3 bagian: angle, distance, height
        float angle = parts[0].toFloat();
        float distance = parts[1].toFloat();
        float height = parts[2].toFloat(); // Nilai ketinggian yang diterima
        ...
        ui->heightLabel->setText(QString("%1 cm").arg(height, 0, 'f', 1)); // Tampilkan ketinggian
...
void MainWindow::updateDetectionPoint(float angle, float distance, float height) {
    qDebug() << "Updating detection point at angle:" << angle << "distance:" << distance << "height:" << height;
    if (distance > 0 && distance <= 200) {
        float x = (distance / 200.0) * 445.0 * qCos(qDegreesToRadians(angle));
        float y = (height / 200.0) * 445.0 * qSin(qDegreesToRadians(angle)); // Gunakan ketinggian untuk menghitung posisi Y
